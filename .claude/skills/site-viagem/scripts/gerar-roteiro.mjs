...
// Casa vem do trip.json (que vem do perfil): moeda e cidade para converter e
// para dizer "abre à venda às 2h em Lisboa" sem assumir país nenhum.
...
secao('Reservar antes', (v.reservas || [])
  ...
  .map((r, i) => {
    ...
    const emCasa = r.abreVendaEm?.emCasa || r.abreVendaEm?.noBrasil
    if (emCasa) {
      linha += `\n   Abre à venda ${dt(emCasa)} às ${emCasa.slice(11, 16).replace(':', 'h')} (${ondeCasa}).`
    }
...
if (o?.categorias?.length) {
  ...
  if (o.cambioReferencia?.length) {
    orc.push('', '_Câmbio de referência: ' + o.cambioReferencia.map((c) =>
      ...
      '. Compras no cartão saem acima disso por causa da taxa de câmbio._')
  }
  secao('Orçamento', orc)
}
